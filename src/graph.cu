 ...
 * @details This file contains the implementation of the Graph class meant for Traveling
...
int read_2DI(int *array, int row, int col, int dim2) { return array[row * dim2 + col]; }
...
int make_node_list(std::string filename, float *node_list) {
  // after the line "NODE_COORD_SECTION", store the x and y coordinates of the cities in
  // node_list csv file format: city number, x coordinate, y coordinate
...
void make_adjacency_matrix(int num_nodes, float *node_list, float *adjacency_matrix) {
  // for each pair of cities, calculate the distance between them and store it in the
  // adjacency matrix the distance between two cities is the Euclidean distance between
  // them the adjacency matrix is symmetric
...
int get_unvisited_neighbors(int num_nodes, float *adjacency_matrix, int node,
                            int *neighbors, bool *visited) {
  // get the unvisited neighbors of a node
  // the unvisited neighbors of a node are the nodes that are connected to it by an edge
  // and have not been visited yet the unvisited neighbors of a node are stored in the
  // neighbors array if there are no unvisited neighbors, return -1. Otherwise, return the
...
__host__ __device__ float calc_path_length(int num_nodes, float *adjacency_matrix,
  ...
  if (num_nodes != path_size) {
    // fprintf(stderr, "err in calc_path_len: num_nodes != path_size (this should not
    // happen)\n");
    return -1;
  }
  ...
  // length += read_2D(adjacency_matrix, path[path_size - 1], path[0], num_nodes); // I
...
void print_node_list(int num_nodes, float *node_list) {
  ...
  for (int i = 0; i < num_nodes; i++) {
    std::cout << "City " << i << ": (" << read_2D(node_list, i, 0, 2) << ", "
              << read_2D(node_list, i, 1, 2) << ")" << std::endl;
  }
}
